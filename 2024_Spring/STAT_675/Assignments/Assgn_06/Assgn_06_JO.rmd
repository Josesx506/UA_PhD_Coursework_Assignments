...
data_plot <- data.frame(
  ...
  Bias = c(metrics$Biases_theta1, metrics$Biases_theta2)
)
ggplot(data_plot, aes(x = Bias, fill = Estimator)) +
  geom_density(alpha = 0.5) +
  labs(title = bquote(paste("Biases for ", theta[1], " & ", theta[2],
                            " (n = ", .(n), ")", sep = ""))) +
...
plot_list <- lapply(seq_along(results), function(n, i) {
  ...
  data_plot <- data.frame(
    Estimator = rep(c("Theta1", "Theta2"), each = replicates),
    Bias = c(df$Biases_theta1, df$Biases_theta2),
...
# (a)
n <- 15
samples <- replicate(1000, rnorm(n, mean = 5, sd = 5))
conf_intervals <- apply(samples, 2, function(x) t.test(x)$conf.int)
avg_length <- mean(diff(t(conf_intervals)))
...
for (mu in seq(2, 2.5, by = 0.01)) {
  type_II_error_rate <- 1 - hypot_test(mu = mu,
                                                    n = 20,
...
for (n in sample_sizes) {
  type_II_error_rate <- 1 - hypot_test(mu = 2.1,
                                                    n = n,
                                                    threshold = qnorm(0.95))
