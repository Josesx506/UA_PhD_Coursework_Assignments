	   ...
	   {count=0;
	    ...
	    while (count>0)
	     {pop();
          ...
          if ((BCmask[ic][jc]!=1)&&(BCmask[ic][jc]!=2)&&(topo[ic][jc]<=max)&&(BCmask[i][j]!=5)&&(count<stacksize))
...
{	
	...
	if (BCmask[iup[i][j]][j]!=5) slope1=topo[i][j]-topo[iup[i][j]][j];if (BCmask[idown[i][j]][j]!=5) slope2=topo[i][j]-topo[idown[i][j]][j];if (BCmask[i][jup[i][j]]!=5) slope3=topo[i][j]-topo[i][jup[i][j]];if (BCmask[i][jdown[i][j]]!=5) slope4=topo[i][j]-topo[i][jdown[i][j]];
...
{	FILE *fp0;
	...
	fp0=fopen("./BCmask.txt","r"); //BCmask = 0 (no BC), 1 (Dirichlet), 2 (Neumann), 3 (periodic), 4 (drainage basin), 5 (exterior of drainage basin)
	...
	for (j=1;j<=lattice_size_y;j++)
	 for (i=1;i<=lattice_size_x;i++) 
	  {fscanf(fp0,"%d",&faultmask[i][j]); 
	   ...
	   Uinitial[i][j]=U[i][j];}
    fclose(fp0);
	for (j=1;j<=lattice_size_y;j++) for (i=1;i<=lattice_size_x;i++) if ((BCmask[i][j]==4)&&((BCmask[iup[i][j]][j]==5)||(BCmask[idown[i][j]][j]==5)||(BCmask[i][jup[i][j]]==5)||(BCmask[i][jdown[i][j]]==5)||(BCmask[iup[i][j]][jup[i][j]]==5)||(BCmask[iup[i][j]][jdown[i][j]]==5)||(BCmask[idown[i][j]][jup[i][j]]==5)||(BCmask[idown[i][j]][jdown[i][j]]==5))) divideadjacenttomaskedge[i][j]=1; else divideadjacenttomaskedge[i][j]=0;  
	uplandarea=0;
...
{   
	...
	while (t>1)
     {t--;
      ...
      if (i==lattice_size_x) j--;
	  fup=0;fdown=0;
	  if ((BCmask[i][j]==0)||(BCmask[i][j]==3)||(BCmask[i][j]==4))
...
/* solves for the value of topo[i][j] for which colluvial sediment is conserved */ 
{   
	...
	if (BCmask[iup[i][j]][j]!=5) slope1=(x-topo[iup[i][j]][j])*oneoverdeltax;if (BCmask[idown[i][j]][j]!=5) slope2=(x-topo[idown[i][j]][j])*oneoverdeltax;if (BCmask[i][jup[i][j]]!=5) slope3=(x-topo[i][jup[i][j]])*oneoverdeltax;if (BCmask[i][jdown[i][j]]!=5) slope4=(x-topo[i][jdown[i][j]])*oneoverdeltax;if (BCmask[iup[i][j]][jup[i][j]]!=5) slope5=(x-topo[iup[i][j]][jup[i][j]])*oneoversqrt2*oneoverdeltax;if (BCmask[iup[i][j]][jdown[i][j]]!=5) slope6=(x-topo[iup[i][j]][jdown[i][j]])*oneoversqrt2*oneoverdeltax;if (BCmask[idown[i][j]][jup[i][j]]!=5) slope7=(x-topo[idown[i][j]][jup[i][j]])*oneoversqrt2*oneoverdeltax;if (BCmask[idown[i][j]][jdown[i][j]]!=5) slope8=(x-topo[idown[i][j]][jdown[i][j]])*oneoversqrt2*oneoverdeltax;
	...
	weatheringrate[i][j]=maximumweatheringratefunction(i,j)*cosfactor*exp(-soil[i][j]/(h_0*cosfactor));
...
/* compute bounds of conservation of mass equation based on max and min of neighboring slopes */
{
	...
	if (topoafterDL[i][j]>max) max=topoafterDL[i][j];if (topoafterDL[iup[i][j]][j]>max) max=topoafterDL[iup[i][j]][j];if (topoafterDL[idown[i][j]][j]>max) max=topoafterDL[idown[i][j]][j];if (topoafterDL[i][jup[i][j]]>max) max=topoafterDL[i][jup[i][j]];if (topoafterDL[i][jdown[i][j]]>max) max=topoafterDL[i][jdown[i][j]];if (topoafterDL[iup[i][j]][jup[i][j]]>max) max=topoafterDL[iup[i][j]][jup[i][j]];if (topoafterDL[idown[i][j]][jup[i][j]]>max) max=topoafterDL[idown[i][j]][jup[i][j]];if (topoafterDL[iup[i][j]][jdown[i][j]]>max) max=topoafterDL[iup[i][j]][jdown[i][j]];if (topoafterDL[idown[i][j]][jdown[i][j]]>max) max=topoafterDL[idown[i][j]][jdown[i][j]];
    if (topoafterDL[i][j]<min) {min=topoafterDL[i][j];mini=i;minj=j;}if (topoafterDL[iup[i][j]][j]<min) {min=topoafterDL[iup[i][j]][j];mini=iup[i][j];diag=1;}if (topoafterDL[idown[i][j]][j]<min) {min=topoafterDL[idown[i][j]][j];mini=idown[i][j];minj=j;diag=1;}if (topoafterDL[i][jup[i][j]]<min) {min=topoafterDL[i][jup[i][j]];mini=i;minj=jup[i][j];diag=1;}if (topoafterDL[i][jdown[i][j]]<min) {min=topoafterDL[i][jdown[i][j]];mini=i;minj=jdown[i][j];diag=1;}if (topoafterDL[iup[i][j]][jup[i][j]]<min) {min=topoafterDL[iup[i][j]][jup[i][j]];mini=iup[i][j];minj=jup[i][j];diag=sqrt2;}if (topoafterDL[idown[i][j]][jup[i][j]]<min) {min=topoafterDL[idown[i][j]][jup[i][j]];mini=idown[i][j];minj=jup[i][j];diag=sqrt2;}if (topoafterDL[iup[i][j]][jdown[i][j]]<min) {min=topoafterDL[iup[i][j]][jdown[i][j]];mini=iup[i][j];minj=jdown[i][j];diag=sqrt2;}if (topoafterDL[idown[i][j]][jdown[i][j]]<min) {min=topoafterDL[idown[i][j]][jdown[i][j]];mini=idown[i][j];minj=jdown[i][j];diag=sqrt2;}	  
	if ((draindiri[i][j]>=1)&&(draindiri[i][j]<=lattice_size_x)&&(draindirj[i][j]>=1)&&(draindirj[i][j]<=lattice_size_y)) 
	 maxtopo=0.5*(topoafterDL[draindiri[i][j]][j]+topoafterDL[i][draindirj[i][j]]+sqrt(SQR(topoafterDL[draindiri[i][j]][j]+topoafterDL[i][draindirj[i][j]])-2*(SQR(topoafterDL[draindiri[i][j]][j])+SQR(topoafterDL[i][draindirj[i][j]])-SQR(fractionnearS_c*S_c*deltax)))); 	  
	else maxtopo=topoafterDL[mini][minj]+diag*fractionnearS_c*S_c*deltax;
