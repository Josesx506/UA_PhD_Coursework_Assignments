...
$$
\frac {\partial e_{ij}}{\partial e_{pq}} = 
\delta_{ip}\delta_{jq}
$$
This causes us to lose symmetry with respect to $ip$ and $jq$ on the rhs, but it can be regained with
$$
\frac {\partial e_{ij}}{\partial e_{pq}} = 
\frac {1}{2} \left(\delta_{ip}\delta_{jq} + \delta_{iq}\delta_{jp} \right)
$$
We can use this to compute $\tau_{ij}$ as:

$$
\begin{aligned}
\tau_{ij}(e) &= \frac {\partial W} {\partial e_{ij}} \\
&= \frac {\partial}{\partial e_{ij}} \left(\frac {1}{2}C_{pqrs}e_{pq}e_{rs} \right) \\
&= \frac {1}{2}C_{pqrs} \left(\frac {\partial e_{pq}}{\partial e_{ij}} e_{rs} + 
e_{pq} \frac {\partial e_{rs}}{\partial e_{ij}} \right) \\
&\text{using the first equation} \\
&= \frac {1}{2} C_{pqrs} (\delta_{pi}\delta{qj} e_{pq} + e_{pq} \delta_{ri}\delta_{sj}) \\
&= \frac {1}{2} (C_{ijrs} e_{rs} + C_{pqij} e_{pq})
\end{aligned}
$$

if $C_{pqij} = C_{ijpq}$ we obtain:
