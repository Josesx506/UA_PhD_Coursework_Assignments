...
hist(rv_dt, freq = FALSE, breaks = 30, xlab = "x",
     main = "Direct Transformation Method", col = "lightcoral", xlim = c(0, 1))
lines(y, 1 - abs(y)) # Density  curve
...
t_distribution_generator <- function(n, df) {
  m <- 0
  sd <- 1
  x <- rnorm(n, m, sd)  # Generate standard normal random variables
#   xm <- (1 / n) * sum(x)
#   sm <- (1 / (n - 1)) * sum((xm - x)^2)
  xm <- mean(x)
  sm <- var(x)
...
x <- seq(-5, 5, length = n)
density <- function(x)  dt(x, df = df)

# Plot histogram and density
hist(samples, main = "Histogram of t-distributed samples", breaks = 20)
curve(density, from = -5, to = 5, add = TRUE, col = "red", lwd = 2)
# cat(length(density), "\n")
...
cat(pt(x, df = df), "n")
# curve(pt(x, df = df), add = TRUE, col = "red", lwd = 2)
```
