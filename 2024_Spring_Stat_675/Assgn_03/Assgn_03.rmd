...
cat("The n value that maximizes collatz from 1 <= n <= 100 is",
    which.max(max_100$num_steps))
max_1000 <- max_collatz(1, 1000)
cat("The n value that maximizes collatz from 1 <= n <= 1000 is",
    which.max(max_1000$num_steps))

# (c)
plot(max_1000$n_val, max_1000$num_steps, type = "l", xlab = "n",
...
goldbach_conjecture <- function(even_num, pout = FALSE) {
  ...
  for (i in 2:(even_num / 2)) {
    if (prime_check(i) && prime_check(even_num - i)) {
      ...
      sol <- rbind(sol, data.frame(pair_1 = i,
                                   pair_2 = even_num - i))
    }
  }
  if (pout) {
...
plot(sol$n, sol$nof, type = "p", xlab = "Even number", col = "#00d5ff",
...
# plot solutions; alternating solutions
plot(sol$n, sol$nof, type = "p", xlab = "Number", col = "#1e9155",
...
small_set <- rnorm(1e2)
med_set <- rnorm(1e3)
small_bench <- microbenchmark(max_sum_subarr_dp(small_set),
                              max_sum_subarr_nn(small_set))
med_bench <- microbenchmark(max_sum_subarr_dp(med_set),
